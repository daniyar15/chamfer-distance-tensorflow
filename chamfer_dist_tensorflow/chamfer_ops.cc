...
// --------------------------------------------------------------------------
// 3. L1 Distance Forward Op Registration
// --------------------------------------------------------------------------

// Forward declaration for L1 kernels
void ChamferDistL1KernelLauncher(int b, int n, const float* xyz,
                                 int m, const float* xyz2,
                                 float* result, int* result_i, float* result2, int* result2_i,
                                 const Eigen::GpuDevice& d);
...
class ChamferDistL1Op : public OpKernel {
public:
    explicit ChamferDistL1Op(OpKernelConstruction* context) : OpKernel(context) {}
    ...
    void Compute(OpKernelContext* context) override {
        ...
        ChamferDistL1KernelLauncher(b, n, xyz1_flat.data(),
                                    m, xyz2_flat.data(),
                                    dist1_flat.data(), idx1_flat.data(),
                                    dist2_flat.data(), idx2_flat.data(),
                                    context->eigen_device<Eigen::GpuDevice>());
...
REGISTER_KERNEL_BUILDER(Name("ChamferDistL1").Device(DEVICE_GPU), ChamferDistL1Op);

// --------------------------------------------------------------------------
// 4. L1 Distance Gradient Op Registration
// --------------------------------------------------------------------------
REGISTER_OP("ChamferDistL1Grad")
    .Input("xyz1: float32")
    .Input("xyz2: float32")
    .Input("idx1: int32")
    .Input("idx2: int32")
...
class ChamferDistL1GradOp : public OpKernel {
public:
    explicit ChamferDistL1GradOp(OpKernelConstruction* context) : OpKernel(context) {}
    ...
    void Compute(OpKernelContext* context) override {
        ...
        ChamferDistL1GradKernelLauncher(b, n, xyz1.flat<float>().data(),
                                        m, xyz2.flat<float>().data(),
                                        grad_dist1.flat<float>().data(), idx1.flat<int>().data(),
                                        grad_dist2.flat<float>().data(), idx2.flat<int>().data(),
                                        grad_xyz1->flat<float>().data(),
...
REGISTER_KERNEL_BUILDER(Name("ChamferDistL1Grad").Device(DEVICE_GPU), ChamferDistL1GradOp);