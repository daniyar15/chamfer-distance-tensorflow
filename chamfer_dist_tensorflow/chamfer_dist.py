...
def compute_distances_l1(
    xyz1: tf.Tensor, xyz2: tf.Tensor
) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor]:
    """
    Computes the minimum L1 (Manhattan) distances between points in two point 
    clouds.
    ...
    Returns:
        A tuple containing
            - dist1: (B, N) L1 distances from xyz1 to xyz2
            - dist2: (B, M) L1 distances from xyz2 to xyz1
            - idx1: (B, N) nearest neighbor indices from xyz1 to xyz2
            - idx2: (B, M) nearest neighbor indices from xyz2 to xyz1
    """
    return _chamfer_module.chamfer_dist_l1(xyz1, xyz2)


@tf.RegisterGradient("ChamferDistL1")
def _compute_distances_l1_grad(op, grad_dist1, grad_dist2, grad_idx1, grad_idx2):
    xyz1 = op.inputs[0]
    xyz2 = op.inputs[1]
    idx1 = op.outputs[2]
    idx2 = op.outputs[3]
    ...
    grad_xyz1, grad_xyz2 = _chamfer_module.chamfer_dist_l1_grad(
        xyz1, xyz2, idx1, idx2, grad_dist1, grad_dist2
    )
